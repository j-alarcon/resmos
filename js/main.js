...
function filterReaction(currentReaction, start) {
  let values;

  // Boolean can decide if the assignment is before or after the loop
  if (start) {
    do {
      values = computerGenerateReaction(...currentReaction);
    } while (
      getReaction(values[0].name, values[1].name, reactions) ===
      lastReactions[1]
    );
  } else {
    while (
  ...
  lastReactions[1] = getReaction(values[0].name, values[1].name, reactions);
  return values;
}
...
function computerPlay(elements, difficulty, playerReaction) {
  ...
  switch (difficulty) {
    ...
    case "normal":
      ...
      values = filterReaction([values, elements], true);
      break;
    ...
    case "hard":
      if (Math.round(Math.random() * 10 > 4)) {
        ...
        values = filterReaction([values, elements], false);
      }
      break;
  }
  return values;
